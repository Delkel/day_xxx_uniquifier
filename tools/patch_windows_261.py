...
# Windows platform support from prior stable build.
lines=s.splitlines()
for i,line in enumerate(lines[:100]):
    if line.startswith('from PySide6.QtCore import ') and 'QUrl' not in line: lines[i]=line.rstrip()+', QUrl'
    if line.startswith('from PySide6.QtGui import ') and 'QDesktopServices' not in line: lines[i]=line.rstrip()+', QDesktopServices'
s='\n'.join(lines)+('\n' if s.endswith('\n') else '')
...
if start>=0:
    ...
    if end>start:
        s=s[:start]+'''    def open_path(self,path):
    ...
    if end>start:
        ...
        try:
            ...
            if not url:
                ...
                return
            if QMessageBox.question(self,"Доступно обновление",f"Доступна версия {latest}. Скачать и установить?")!=QMessageBox.Yes: return
...
# Preserve compact 2.6.1 UI and make footer updater use same button styling.

...
ICON, LOGO = RESOURCES / "EnkiAgencyUniq.icns", RESOURCES / "EnkiAgencyUniq.png"
SETTINGS_PATH = ROOT / "settings.json"
UPDATE_MANIFEST_URL = "https://raw.githubusercontent.com/Delkel/day_xxx_uniquifier/main/update-manifest.json"
TELEGRAM_BOT_URL = "https://t.me/enki_agency_bot"
...
def fetch_update_manifest(url: str) -> dict:
    ...
    if not isinstance(data,dict) or not data.get('version') or not data.get('zip_url'):
        raise RuntimeError("В манифесте нужны поля version и zip_url")
    return data


def write_update_helper(manifest: dict) -> Path:
    d=Path(tempfile.mkdtemp(prefix='enki_update_')); p=d/'update.command'
...
curl -fL {json.dumps(str(manifest['zip_url']))} -o "$Z"
...
open "$A"
''', encoding='utf-8'); p.chmod(0o755); return p
...
class MainWindow(QMainWindow):
    ...
    def build_ui(self):
        ...
        sidebar=QFrame(objectName='sidebar'); sidebar.setFixedWidth(255); side=QVBoxLayout(sidebar); side.setContentsMargins(18,18,18,18); side.setSpacing(5)
        ...
        actions=[
            ...
            ("Перейти в команду",self.open_team),]
    ...
    def open_team(self):
        webbrowser.open(TELEGRAM_BOT_URL, new=2)
...
""")
    ...
    def check_updates(self):
        url=str(self.settings.get('updateManifestUrl') or UPDATE_MANIFEST_URL).strip(); self.compact_update_btn.setEnabled(False);self.ready.setText('●  Проверяю обновление…');self.update_worker=UpdateWorker(url)
        self.update_worker.checked.connect(self.on_update_checked);self.update_worker.failed.connect(self.on_update_failed);self.update_worker.finished.connect(lambda:self.compact_update_btn.setEnabled(True));self.update_worker.start()
    ...
    def on_update_failed(self,e):self.compact_update_btn.setEnabled(True);self.ready.setText('●  Готов к работе');QMessageBox.critical(self,'Ошибка обновления',f'Не удалось проверить обновления:\n{e}')
    def open_path(self,path): subprocess.Popen(['open',str(path)])
